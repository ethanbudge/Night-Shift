...
- **Pro plan realities:** overnight runs use your default model (Sonnet) unless `MODEL` is set in `config.env`; Opus isn't part of Pro, and setting it there will fail without a Max plan. Long tasks may span multiple nights via WIP branches — that's by design.
