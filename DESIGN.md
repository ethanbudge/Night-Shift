...
GitHub's native notifications do **not** work here: the agent authenticates with the owner's PAT, its comments are the owner's own activity, and GitHub never notifies you about yourself. Instead, a GitHub Action (`.github/workflows/notify.yml`) fires on three label events plus one comment marker, and curls a push to a private [ntfy.sh](https://ntfy.sh) topic (stored as an Actions secret; the topic name is effectively the password, so make it long and random):
- `status:in-progress` (a task started; the notification includes the resolved model, read from the issue's `model:<tag>` label if it has one, else noted as the account baseline). Only fires on a genuinely fresh pickup (the `status:ready` → `status:in-progress` transition) since GitHub only emits `labeled` for a newly-added label — a multi-hour resume of the same task, already carrying that label, stays silent, avoiding a ping every time a long task's WIP resumes.
- `status:needs-human` (questions) and `status:in-review` (PR ready) — the original two triggers.
- **Review started** (Review System, task #4): the review pass can't signal this via a label — it's reviewing an issue that's already `status:in-review`, and only one `status:*` label is allowed per issue, and `night-shift.sh` can't pre-announce which issue will be picked (the reviewing agent only knows that from inside its own invocation). So `REVIEW_PROMPT.md` has the review agent post a fixed marker comment as its first action after picking a candidate — `Starting review pass (escalating <from> -> <to>)` — and the workflow's second job (`issue_comment: created`) watches for that phrase instead of a label event, extracting the escalation models directly from the comment text.

The phone app subscribes to the topic; tapping the notification opens the issue. Everything else (creating tasks, replying, merging) happens in the GitHub mobile app or web. If you ever want hard separation of identities (and native GitHub notifications), a free machine-user account holding the PAT is the upgrade path.
...
## Roadmap: built and applied

Everything below is **built and now folded directly into `repo/ops/`, `repo/.claude/`, `repo/CLAUDE.md`, and `repo/RUNNER_PROMPT.md`** — a fresh clone of `repo/` ships with all of it working, nothing left to apply. `repo/proposed/` is empty (see [its README](proposed/README.md)); the write-ups below are kept as design history.

Why these lived under `repo/proposed/` first, and why the folder still exists: writing to `ops/`, `.claude/`, `.github/`, `CLAUDE.md`, or `RUNNER_PROMPT.md` under `repo/` is permanently write-denied to the Night Shift agent, in every repo it ever touches, including this one (see `RUNNER_PROMPT.md`'s hard rules — it's the same self-protection that stops the agent from loosening its own gates in a hub repo, applied literally). That's a real structural collision: this repo's product *is* a set of files that live at those paths, so the agent stages new versions under `proposed/` and a human folds them in with `repo/apply-roadmap.sh`. These items went through exactly that flow; `proposed/` stays as the staging area for the *next* task that needs it. Everything below was written, and where testable in a macOS sandbox, run and verified, by the agent.

**All of the items below are applied** — cross-platform install, remote control, the secret-leak guard, the original CLI polish (`model`/`logs`), the efficiency pre-check (task #7), the hand-off README (task #10), the Review System (task #4), and per-task Model Tags (task #12) have all been folded into `ops/`/`.claude/`/`CLAUDE.md`/`RUNNER_PROMPT.md` on `main`; none of them live under `proposed/` anymore. The write-ups below are kept as design history.
...
**Hand-off README.** Built: `proposed/runner/update_handoff.py`, stdlib-only, unit-tested in this sandbox (upsert, remove, sync-to-empty, sync-to-partial, and fresh-file creation all verified). Supersedes the "nightly digest issue" idea below with something cheaper to keep current: a `README.md` at the hub repo root always reflects, in one glance, every task currently waiting on the owner — a PR to review, questions to answer, or a `HANDOFF.md` checklist to work through — regardless of which repo the work actually landed in. The script rewrites one marked block from a JSON comment it keeps as the source of truth, so no LLM tokens are spent re-generating prose on every run; the agent only ever passes short structured arguments (issue number, one-line action) it already has in hand at each end state. The `RUNNER_PROMPT.md` wiring (sync stale entries during task selection, upsert one on every end state) is a manual patch described in `proposed/runner-prompt-patch-handoff.md`, same reasoning as the secret-scan patch — this one edit can't be automated even by `apply-roadmap.sh`.

**Review System.** Built: `proposed/runner/night-shift.sh` (a review sub-loop appended after the normal task loop), `proposed/runner/mode.sh` (`nightshift review [on|off|status]`), `proposed/runner/config.env` (the `REVIEW_MODE` knob, default `off`), `proposed/agent-settings/settings.json` and `proposed/runner/runner-settings.json` (add the new `REVIEW_PROMPT.md` to the same deny lists that already protect `CLAUDE.md`/`RUNNER_PROMPT.md`/`CONTROL.md`), and [`repo/REVIEW_PROMPT.md`](REVIEW_PROMPT.md) itself — which ships live at the repo root already, the same way `CONTROL.md` did, since a brand-new filename isn't a protected path until the settings wiring above makes it one.

The idea (from the "More Efficient Night Shift" / "Review System" backlog item): idle end-of-queue credits are wasted if the queue is genuinely empty but the weekly budget still has room. Instead of exiting, give the highest-priority already-completed-but-unreviewed task one more pass from a stronger model before the run ends.

- **Trigger:** only when `REVIEW_MODE=on` *and* the normal task loop broke on `NO_ACTIONABLE_TASKS` (a real empty queue — not merely out of budget, wall-clock, or the per-run task cap). `night-shift.sh` tracks this with a `queue_empty` flag set only on that specific break, so a loop that stopped early for any other reason never triggers a review pass.
- **What counts as a candidate:** `night-shift.sh` already parses each task invocation's `TASK_COMPLETE <n> <letter>` marker; every end-state-A (finished) completion now also appends a `timestamp,issue` row to `$LOG_DIR/completions.csv`. A completed issue counts as "unreviewed" until its number appears in `$LOG_DIR/reviewed.csv`. Both files live in `LOG_DIR`, same as `usage-log.csv` — outside the sandboxed repo clone, so a review invocation (which runs inside the same sandbox as a normal task) can't self-mark itself reviewed; only the wrapper does that, by parsing a `REVIEW_COMPLETE <n>` marker the same way it parses `TASK_COMPLETE`.
- **Candidate selection:** the wrapper doesn't rank by priority itself — it hands the reviewing agent the full pending list (oldest completion first) and lets it fetch each candidate's `priority:*` label via the GitHub API and pick the highest, exactly mirroring how the normal `RUNNER_PROMPT.md` task-selection step already works. Keeps the bash wrapper dumb and the "smart" selection logic in one place (the prompt), not duplicated in two languages.
- **Model escalation ladder:** fixed, not configurable — `sonnet` (or blank, i.e. account default) `-> opus -> fable`. A review pass with the base model already at `fable` has nowhere to escalate to and is skipped with a log line rather than reviewing with the same model that wrote the code.
- **What the reviewer does:** [`REVIEW_PROMPT.md`](REVIEW_PROMPT.md) is a sibling to `RUNNER_PROMPT.md` — same environment facts, same hard rules, same agent-comment header — but its job is to find the PR that closed the chosen issue (by `head.ref` prefix `task/<n>-`, since that branch-naming convention is already unique per issue), read its diff, and either leave a short "looks correct" comment or push a fix (to the same branch if the PR is still open, or a new `task/<n>-review-fix` branch/PR if it already merged). It always ends by printing `REVIEW_COMPLETE <n>` — even a review that finds nothing to fix, or finds no PR at all — so the issue is never picked again.
- **Shared per-run cap:** review invocations count against the same `MAX_TASKS_PER_RUN` the normal task loop uses, deliberately — no separate config surface, and it keeps the worst-case number of `claude -p` invocations per hourly wake-up bounded exactly as before.
- **Tested:** the model-escalation ladder and the pending-candidate dedup logic were unit-tested directly (both are small, pure functions extracted as inline Python in `night-shift.sh`, tested in isolation from a scratch fixture). `apply-roadmap.sh` was dry-run against a scratch copy of `repo/` to confirm it folds the staged files into `ops/`/`.claude/` correctly without disturbing untouched files. `nightshift review on/off/status` was run against a scratch install. The end-to-end review invocation itself (an actual `claude -p` call against `REVIEW_PROMPT.md`) could not be exercised in this sandbox — same caveat as the rest of this roadmap section, since that requires a live run against real GitHub issues/PRs.

**Per-task model tags.** Built: a `model:<tag>` label (e.g. `model:opus`, `model:sonnet`, `model:haiku`, `model:fable` — already created on the hub repo) lets a task issue request a specific model instead of every task running under one account-wide default. `check_budget.py` gained `select_next_task()` — a best-effort mirror of `RUNNER_PROMPT.md`'s "Select one task" algorithm, run from outside the agent (it has to be: the model is a `claude` CLI flag, fixed before the process starts, so nothing inside a running session can change it) — and `resolve_task_model()`, which looks the picked issue's `model:<tag>` label up in `MODEL_ALLOWLIST` (a new `tag=model-id,...` knob in `config.env`) and returns the resolved model ID, or nothing if the issue has no tag or an unrecognized one. `night-shift.sh`'s per-iteration model resolution now has four tiers: `CONTROL.md`'s `model:` line (unchanged — still the "I'm away, override everything right now" channel) beats the picked task's own `model:<tag>` (riding along in the guard's decision JSON as `task_model_id`), which beats `MODEL` in `config.env` (the baseline every task without a tag, or with an unresolvable one, quietly falls back to — never a hard failure), which beats claude's account default. The first two tiers resolve in the shared `effective_model_for()` helper — the same one the review pass uses, so both loops agree on precedence (during a review the queue is empty, so no task is selected and `task_model_id` is absent, collapsing the helper back to just the `CONTROL.md` override). All of the pure logic (`parse_model_allowlist`, `priority_rank`, `model_tag`, `is_agent_comment`, the `Depends on #N` regex, and `select_next_task`'s three tiers against mocked GitHub responses — owner-replied needs-human, stale vs. fresh in-progress, priority/age ordering within ready, and both directions of a `Depends on` check) is unit-tested in this sandbox; the live GitHub calls themselves are the same untestable-without-a-deployed-instance case as `CONTROL.md`'s fetch, and fail the same safe way (a fetch error just makes that tier come up empty, never blocks or crashes the guard). A `nightshift update-models` command (`proposed/runner/mode.sh`) uses a cheap, read-only `claude -p` call (web search only, no repo access) to check for Claude model releases not yet in `MODEL_ALLOWLIST`, creates the corresponding hub labels directly (an ordinary label write, not a protected-path edit), and prints the `MODEL_ALLOWLIST` line for you to hand-apply. **One correctness fix this batch made to a protected file:** `CLAUDE.md`'s "replace labels" guidance only mentioned preserving the priority label, which meant every routine label-set PUT (claiming a task, any end-state transition) would have silently deleted a task's `model:*` label the first time an agent touched it — described in `proposed/runner-prompt-patch.md` along with the (much smaller) `RUNNER_PROMPT.md` note about model resolution happening before the agent starts.
