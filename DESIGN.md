...
These are specified in enough detail to implement directly. They're written up here rather than shipped because building them requires editing `ops/`, `.claude/`, `.github/`, `CLAUDE.md`, or `RUNNER_PROMPT.md` under `repo/` — and those exact path components are permanently write-denied to the Night Shift agent, in every repo it ever touches, including this one (see `RUNNER_PROMPT.md`'s hard rules — it's the same self-protection that stops the agent from loosening its own gates in a hub repo, applied literally). That's a real structural collision: this repo's product *is* a set of files that live at those paths. Implementing the roadmap below is manual work for a maintainer, or requires a session running under different rules than Night Shift's own agent.

**Cross-platform install.** Today `ops/install.sh` only understands macOS (`launchd`, `com.claude.nightshift.plist`, `caffeinate`, `security` for Keychain). To reach Linux and Windows without forking the guard logic:
...
**Secret-leak guard.** DoD asks that if Night Shift finds a credential accidentally committed in a target repo, it stops editing that repo and flags it rather than continuing. Proposed shape: `ops/secret_scan.py`, stdlib-only, scanning a freshly cloned `workspaces/<name>` for common token shapes (`ghp_`/`github_pat_`/`sk-ant-`/AWS access keys/PEM private-key headers) plus a generic high-entropy-assignment heuristic, skipping `.git/` and binary files, printing `path:line: <pattern-name>` **without** ever printing the matched value, exit 0 clean / 1 findings. `RUNNER_PROMPT.md`'s "Locate the work" step would run it immediately after cloning and before any edit, and a nonzero exit is a hard stop on that task — flag `status:needs-human`, name the file and pattern (never the value), recommend the owner rotate/remove it, and don't touch the repo further.

**CLI polish.** `ops/mode.sh` (the `nightshift` command) is a reasonable base — it already handles BSD/GNU `date` portably. Two straightforward additions: `nightshift model [<id>|default]` to flip `MODEL` in `config.env` without hand-editing it (mirrors the existing day-off/vacation UX), and `nightshift logs` to tail `night-shift.log`. An `ops/uninstall.sh` (reverse of `install.sh`: unload the launchd job / disable the systemd timer, remove the symlink, leave `secrets/`+`logs/` untouched unless `--purge`) would round out a project people other than the original author are expected to install and remove.

**Housekeeping already identified but not fixable here (all under the blocked paths):** `ops/night-shift.sh` and `ops/mode.sh` hardcode `/usr/bin/python3`; a plain `python3` (PATH lookup) is what actually needs to be portable across Linux distros and is consistent with `check_budget.py`'s own shebang. `ops/config.env`'s `CLAUDE_BIN` default assumes the native installer's path — worth a comment noting `command -v claude` as the general fallback for non-default installs.
